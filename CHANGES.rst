...
* Skipped scenarios (e.g. via ``@pytest.mark.skip``, a ``@skip`` tag in the feature file, or a skip raised in a step) are now reported in the cucumber json output instead of being omitted.
